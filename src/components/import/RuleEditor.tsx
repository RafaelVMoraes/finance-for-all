...
import { ImportRule, RuleCondition, RuleActions, ConditionType } from '@/types/importRules';
...
export function RuleEditor({ rule, categories, onSave, onClose, open }: RuleEditorProps) {
  ...
  const updateCondition = (index: number, field: 'type' | 'value', value: string | number | boolean) => {
  ...
  const handleSave = async () => {
    ...
    if (!categoryId && !duplicateAction) {
      setError('At least one action (category or duplicate handling) is required');
      return;
    }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {conditions.length === 0 && (
              <p className="text-sm text-muted-foreground">No conditions yet. Add at least one.</p>
            )}
            
            {conditions.map((condition, index) => (
              <div key={index} className="flex gap-2 items-center rounded-lg border p-2 bg-muted/50">
                <Select
                ...
                {(condition.type === 'value_min' || condition.type === 'value_max') && (
                  <Input
                    type="number"
                    value={String(condition.value)}
                ...
                {condition.type === 'value_sign' && (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="income">Income (value &gt; 0)</SelectItem>
                      <SelectItem value="expense">Expense (value &lt; 0)</SelectItem>
                    </SelectContent>
                  </Select>
                )}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Label className="text-sm text-muted-foreground">Assign Category</Label>
              <Select
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setDuplicateAction(v === '__none__' ? null : v)}
              >
                <SelectTrigger>
